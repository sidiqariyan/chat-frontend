...
const TABS = ['Company', 'Services', 'Resources', 'Account']

export default function NavBar() {
  const [open, setOpen] = useState(false)
  const [tab, setTab] = useState('Company')
  const onEnter = (t) => { setTab(t); setOpen(true) }
  const onLeave = () => setOpen(false)
  const onToggle = (t) => { setTab(t); setOpen(v => !v) }
      ...
      <div className="container">
        ...
        <div className="navbar-collapse d-flex align-items-center justify-content-between" id="navbarSupportedContent">
          <ul className="navbar-nav menu ms-auto d-flex align-items-center" style={{gap:16}}>
            {TABS.map((t) => (
              <li key={t} className={`nav-item dropdown submenu position-static ${open && tab===t ? 'show':''}`} onMouseEnter={()=>onEnter(t)} onMouseLeave={onLeave}>
                <button type="button" className="nav-link dropdown-toggle btn btn-link p-0" aria-haspopup="true" aria-expanded={open && tab===t} onClick={()=>onToggle(t)} onTouchStart={()=>onToggle(t)}>
                  {t}
                </button>
                <i className="fa fa-angle-down mobile_dropdown_icon"></i>
                <ul className={`dropdown-menu p-4 mega_menu ${open && tab===t ? 'show':''}`} style={dropdownStyle}>
                  <div className="container">
                    <div className="row">
